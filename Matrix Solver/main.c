...
int main(int argc, char **argv)
{
  ...
  genmatrix(n, A);
  genvector(n, b);
// Save A and b so we use it to check the result. memmove(B,A,n*n*sizeof(double)); memmove(c,b,n*sizeof(double));
// You can overwrite A and b.
  int success = linsolve(n, A, b, x); checkresult(n, success , B, c, x);
  return 0;
}
